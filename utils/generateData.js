...
const getData = async () => {
    ...
    console.log(baseDir)
    const BASE_URL = 'https://hamariweb.com/islam/pakistan_ramadan-timings1.aspx?Page=';
    let cities = [];
    ...
    try {
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            $("#Region").find("tbody > tr").find("td > a").each(function () {
                let city = $(this).text().replace(/[•\t.+]/g, '').trim().toLowerCase().replace(/\s+/g, '-');
                let href = $(this).attr("href");
                citiesToSaveInFile.push(city);
                cities.push({ city, timetableLink: `https://hamariweb.com/islam/${href}` });
            });
        }
        ...
        for (let city of cities) {
            ...
            fs.writeFile(`${baseDir}/${city.city}.json`, JSON.stringify(data), 'utf8', (err) => {
                if (err) throw err;
